...
class TSDFServer(object):
    ...
    def sensor_cb(self, msg):
        ...
        # --- FIX: Resize the depth image to match the intrinsic resolution (640x480 expected) ---
        h_intr, w_intr = self.intrinsic.height, self.intrinsic.width
        if img.shape[:2] != (h_intr, w_intr):
            rospy.logwarn(
                f"Depth image size {img.shape[:2]} != intrinsic ({h_intr}, {w_intr}), resizing..."
            )
            import cv2
            img = cv2.resize(img, (w_intr, h_intr), interpolation=cv2.INTER_NEAREST)
        # ----------------------------------------------------------------------------------------
